...
for item in data:
    ...
    output_folder_pywt = f"/home/dlserver1/dlserver1_web/all/train/{itype}"
    ...
    if not os.path.exists(output_folder_pywt):
        os.makedirs(output_folder_pywt)
    ...
    for filename in os.listdir(output_folder_rotation):
        if filename.endswith(".jpg") or filename.endswith(".png"):
            ...
            coeffs = pywt.dwt2(image, 'bior1.1')
            LL, (LH, HL, HH) = coeffs
            pywt1_image = plt.imsave(os.path.splitext(input_path)[0]+'LL.png', LL, cmap='gray')
            pywt1_image = plt.imsave(os.path.splitext(input_path)[0]+'LH.png', LH, cmap='gray')
